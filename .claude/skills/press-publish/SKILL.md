...
# /press-publish — Start a template-publish run
...
1. **Production safety**: if `--env prod`, ask the partner to confirm explicitly before invoking. The API will pause the run at the release gate; an approver must call `/press-approve {run_id}` to release to prod.
...
- `--env prod` publishes to test first, then **pauses for approval**. After human review, an approver runs `/press-approve {run_id}` to release.
...
- `/press-status` — poll progress
- `/press-logs` — get the CloudWatch log group for this run
